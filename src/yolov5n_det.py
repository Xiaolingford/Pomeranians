"""
YOLOv5n Detection — Training + Evaluation + Benchmark
======================================================
YOLOv5 nano baseline for microplastic detection.
Uses Ultralytics native pipeline with settings matched as closely
as possible to the LDBST-MP training recipe.

Note: YOLOv5n is loaded via Ultralytics' unified API (same as YOLOv8).
The Ultralytics package supports YOLOv5 models natively.

Usage:
    # Train + benchmark
    python src/yolov5n_det.py --task both --data data.yaml --epochs 100 --batch 16 --img_size 224

    # Train only
    python src/yolov5n_det.py --task train --data data.yaml --epochs 100

    # Benchmark only (requires --weights)
    python src/yolov5n_det.py --task benchmark --data data.yaml --weights runs/detect/yolov5n/weights/best.pt
"""
...
def train_model(data_yaml: str, epochs: int, batch: int, img_size: int,
    ...
    # YOLOv5n via Ultralytics unified API
    weights = "yolov5nu.pt" if pretrained else "yolov5n.yaml"
    model = YOLO(weights)

    print(f"\n[model] Loaded: {weights}")
    print(f"[note]  yolov5nu.pt = YOLOv5n Ultralytics (updated anchors + head)")
...
def main():
    ...
    ap.add_argument("--name", default="yolov5n_microplastic")
