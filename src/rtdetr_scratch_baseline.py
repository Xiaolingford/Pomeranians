"""
RT-DETR-L From-Scratch Baseline — Train + Evaluate + Benchmark
================================================================
Trains RT-DETR-L from RANDOM INITIALIZATION (no COCO pretrained
weights) for fair comparison with LDBST-MP (C5 revision).

RT-DETR-L uses an HGNetv2 backbone with a hybrid transformer encoder
and is the smallest RT-DETR variant available through Ultralytics.
It provides a transformer-based contrast to the CNN-based YOLO baselines
and your hybrid LDBST-MP model.

Usage:
    # Train + benchmark
    python src/rtdetr_scratch_baseline.py --task both --data_dir ./data_yolo --epochs 150 --batch 16 --img_size 224

    # Benchmark only (re-benchmark existing weights with cleaner VRAM)
    python src/rtdetr_scratch_baseline.py --task benchmark --weights runs/detect/rtdetr_l_scratch/weights/best.pt --data_dir ./data_yolo
"""
...
# -----------------------------------------------------------------------
# Benchmark (uses same methodology as LDBST-MP: empty_cache + reset-after-warmup)
# -----------------------------------------------------------------------
def benchmark_model(weights_path: str, img_size: int = 224, n_warmup: int = 50, n_runs: int = 500):
    ...
    # Clear cached memory before warmup — matches LDBST-MP benchmark
    if device.type == "cuda":
...
def train_model(data_yaml: str, epochs: int, batch: int, img_size: int,
                device: str, name: str):
    print("\n" + "=" * 70)
    print(f"Training {MODEL_NAME} FROM SCRATCH (C5 fair comparison)")
    print("=" * 70)
    print(f"[config] Architecture: rtdetr-l.yaml  (no pretrained weights)")
    ...
    print(f"\n[model] Loaded architecture from rtdetr-l.yaml — weights randomly initialized")
    print(f"[note]  RT-DETR-L = HGNetv2 backbone + hybrid transformer encoder")
    print(f"[note]  NMS-free end-to-end detection (unlike YOLO)")
...
def save_results(benchmark: dict, detection: dict, out_dir: str):
    ...
    json_path = Path(out_dir) / "rtdetr_l_scratch_benchmark.json"
    with open(json_path, "w") as f:
        json.dump(combined, f, indent=2)

    txt_path = Path(out_dir) / "rtdetr_l_scratch_benchmark.txt"
    with open(txt_path, "w") as f:
        ...
        f.write("Model info\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Architecture: HGNetv2 + Hybrid Transformer Encoder\n")
        f.write(f"  Paradigm:     End-to-end (NMS-free)\n")
...
# -----------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description=f"Train {MODEL_NAME} from scratch for C5 fair comparison")
    ...
    ap.add_argument("--name",     default="rtdetr_l_scratch")
    ...
    print("\n" + "=" * 70)
    print(f"{MODEL_NAME} — From-Scratch Baseline (C5 Revision)")
    print("=" * 70)
    print(f"[info] Transformer-based detector (contrast to CNN-based YOLO)")
    print(f"[info] ~32M params — included as accuracy ceiling reference")
    ...
    if args.task in ("train", "both"):
        best_weights = train_model(
            ...
            img_size=args.img_size, device=args.device, name=args.name,
        )
